...
interface Props {
  ...
  onSubProjectUpdated: (id: string, projectName: string, description: string) => void;
...
const EditParentProjectModal = ({ open, onClose, projectId, currentName, currentDescription,onSubProjectUpdated,onSubProjectDeleted } : Props) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      await SubProjectAPI.updateSubProject(projectId, { 
        projectName: trimmedName,
        description: trimmedDescription
      });

      onSubProjectUpdated(projectId, trimmedName, trimmedDescription);
      handleClose();
    } catch (err: any) {
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      await SubProjectAPI.deleteSubProject(projectId);
      setConfirmDeleteOpen(false);

      if (onSubProjectDeleted) {
        onSubProjectDeleted(projectId);
      }
